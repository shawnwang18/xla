...
CommandBufferCmdExecutor::Dependencies(const RecordParams& record_params,
  ...
  dependency_strings.reserve(dependencies_ids.size());
  for (const auto cmd_id : dependencies_ids) {
    dependency_strings.push_back(
        absl::StrCat(commands_[cmd_id]->ToString(), " ", cmd_id));
  }
  VLOG(2) << "Dependencies for command " << id
          << " are: " << absl::StrJoin(dependency_strings, ", ");
