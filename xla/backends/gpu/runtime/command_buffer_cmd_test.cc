...
TEST(CommandBufferCmdTest, NestedChildCmdCreateAndUpdate) {
  ...
  //   Node 0: MemcpyDeviceToDeviceCmd (MEMCPY a->c) -> no dependencies
  {
    ...
    auto* outer_child_cmd = dynamic_cast<
        const se::gpu::GpuCommandBuffer::GpuChildCommand*>(
    ...
    auto* outer_memzero_cmd = dynamic_cast<
        const se::gpu::GpuCommandBuffer::GpuCommand*>(outer_commands[1].get());
    ...
    auto* outer_empty_cmd = dynamic_cast<
        const se::gpu::GpuCommandBuffer::GpuCommand*>(outer_commands[2].get());
    ...
    auto* middle_child_cmd = dynamic_cast<
        const se::gpu::GpuCommandBuffer::GpuChildCommand*>(
    ...
    auto* middle_memset_cmd = dynamic_cast<
        const se::gpu::GpuCommandBuffer::GpuCommand*>(middle_commands[1].get());
    ...
    auto* middle_memcpy_cmd = dynamic_cast<
        const se::gpu::GpuCommandBuffer::GpuCommand*>(middle_commands[2].get());
    ASSERT_NE(middle_memcpy_cmd, nullptr)
        << "Third middle command should be GpuCommand (MemcpyDeviceToDeviceCmd)";
    ...
    auto* inner_memcpy_cmd = dynamic_cast<
        const se::gpu::GpuCommandBuffer::GpuCommand*>(inner_commands[0].get());
    ...
    ASSERT_NE(inner_memcpy_cmd->handle, nullptr)
        << "Inner MemcpyDeviceToDeviceCmd should have a valid graph node handle";
