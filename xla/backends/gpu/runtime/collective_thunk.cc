...
absl::StatusOr<const se::CommandBuffer::Command*> CollectiveThunk::Record(
  ...
  // NCCL collectives can use internal streams and event dependencies. Capture
  // each collective into an isolated graph so these dependencies do not cross
  // the capture boundary of an already-populated parent command buffer.
