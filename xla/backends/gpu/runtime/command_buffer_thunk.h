...
class CommandBufferThunk : public Thunk {
 ...
 private:
  ...
  // new one. When VA remapping is enabled, the key includes a VA-remapped
  // allocation address to distinguish per-VA-range command buffers; otherwise
  // the key uses nullptr.
  absl::StatusOr<std::shared_ptr<ExecutorCommandBuffer>>
  GetOrCreateCommandBuffer(se::StreamExecutor* executor,
                           const BufferAllocations& buffer_allocations,
                           const CommandBufferUpdateInfo* update_info);
