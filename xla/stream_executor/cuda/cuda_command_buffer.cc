...
absl::Status WriteGraphToDotFileV2(CUgraph graph, absl::string_view path) {
...
absl::StatusOr<NodeHandle> CudaCommandBuffer::CreateChildNode(
  ...
  TF_RETURN_IF_ERROR(WriteGraphToDotFileV2(child_graph, path));
  std::string dot_file_contents;
  TF_RETURN_IF_ERROR(
      tsl::ReadFileToString(tsl::Env::Default(), path, &dot_file_contents));
  VLOG(3) << "Contents of child graph" << path << " is:\n" << dot_file_contents;

  size_t num_nodes;
  TF_RETURN_IF_ERROR(
      cuda::ToStatus(cuGraphGetNodes(child_graph, nullptr, &num_nodes),
                     "Failed to get nodes from CUDA graph"));
  VLOG(3) << "Nodes in the CUDA graph: " << num_nodes;

  std::vector<CUgraphNode> nodes;
  nodes.resize(num_nodes);
  TF_RETURN_IF_ERROR(
      cuda::ToStatus(cuGraphGetNodes(child_graph, nodes.data(), &num_nodes),
                     "Failed to get nodes from CUDA graph"));
  CUgraphNodeType type;
  CUDA_KERNEL_NODE_PARAMS params;
  for (size_t i = 0; i < num_nodes; i++) {
    TF_RETURN_IF_ERROR(
        cuda::ToStatus(cuGraphNodeGetType(nodes[i], &type),
                       "Failed to get node type from CUDA graph"));
    VLOG(3) << "Node " << i << " in the CUDA graph: " << nodes[i]
            << " type: " << type;
    if (type == CU_GRAPH_NODE_TYPE_KERNEL) {
      VLOG(3) << "Node " << i << " is a kernel node";

      TF_RETURN_IF_ERROR(
          cuda::ToStatus(cuGraphKernelNodeGetParams(nodes[i], &params),
                         "Failed to get kernel node params from CUDA graph"));

      VLOG(3) << "Kernel node params: " << params.func << " " << params.gridDimX
              << " " << params.gridDimY << " " << params.gridDimZ << " "
              << params.blockDimX << " " << params.blockDimY << " "
              << params.blockDimZ << " " << params.sharedMemBytes;
    }
  }
...
absl::StatusOr<NodeHandle> CudaCommandBuffer::CreateKernelNode(
  ...
  VLOG(2) << "Created kernel node: " << reinterpret_cast<void*>(node_handle);
  return FromCudaGraphHandle(node_handle);
}
...
absl::StatusOr<NodeHandle> CudaCommandBuffer::CreateEmptyNode(
  ...
  if (stream_executor_->GetDeviceDescription().driver_version() <
    ...
    VLOG(2) << "Create empty node as a no-op kernel node, kernel: "
            << reinterpret_cast<void*>(noop);
...
absl::Status CudaCommandBuffer::Trace(
  ...
  TF_RETURN_IF_ERROR(
      tsl::ReadFileToString(tsl::Env::Default(), path, &dot_file_contents));
  VLOG(3) << "Contents of traced graph" << path << " is:\n"
          << dot_file_contents;
  ...
  size_t numNodes;
  TF_RETURN_IF_ERROR(cuda::ToStatus(cuGraphGetNodes(graph_, nullptr, &numNodes),
                                    "Failed to get nodes from CUDA graph"));
  VLOG(3) << "Nodes in the CUDA graph: " << numNodes;
...
absl::Status CudaCommandBuffer::Finalize() {
  ...
  if (mode() == Mode::kPrimary) {
    ...
    if (exec_ == nullptr) {
      ...
      VLOG(5) << "No operation is needed for updating the executable graph";
    }
  } else {
