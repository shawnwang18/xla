...
absl::Status CudaExecutor::VmmSetAccess(DeviceAddressBase* address,
  ...
  if (address == nullptr || address->is_null()) {
    return absl::InvalidArgumentError(
        "Cannot set VMM access on null address.");
  }
  if (address->size() == 0) {
    return absl::OkStatus();
  }
  ...
  uint64_t size = ((address->size() + granularity - 1) / granularity) * granularity;

  TF_RETURN_IF_ERROR(cuda::ToStatus(
      cuMemSetAccess(ptr, size, &access_desc, count),
      absl::StrFormat("Failed to set VMM access for address %p with size %llu",
                      address->opaque(), static_cast<unsigned long long>(size))));
...
DeviceAddressBase CudaExecutor::ReserveAddress(uint64_t size,
  ...
  CUmemAllocationProp properties =
      GetVmmAllocationProperties(device_, is_rdma_supported_);
  size_t granularity = 0;
  auto granularity_status = cuda::ToStatus(cuMemGetAllocationGranularity(
      &granularity, &properties, CU_MEM_ALLOC_GRANULARITY_RECOMMENDED));
  if (!granularity_status.ok()) {
    XLA_LOG_DEVICE(ERROR, device_ordinal())
        << "Failed to get allocation granularity: " << granularity_status;
    return DeviceAddressBase(nullptr, 0);
  }

  // Reserve virtual address space using cuMemAddressReserve.
  CUdeviceptr ptr = 0;
  auto status = cuda::ToStatus(cuMemAddressReserve(&ptr, size, granularity,
                                                   /*addr=*/0, /*flags=*/0));
  if (!status.ok()) {
    XLA_LOG_DEVICE(ERROR, device_ordinal())
        << "Failed to reserve virtual address of size "
  ...
  XLA_VLOG_DEVICE(2, device_ordinal())
      ...
      << " of size " << size << " bytes with granularity " << granularity;

  return DeviceAddressBase(reinterpret_cast<void*>(ptr), size);
}

void CudaExecutor::FreeAddress(DeviceAddressBase* mem) {
  XLA_VLOG_DEVICE(1, device_ordinal())
  ...
  auto status = cuda::ToStatus(cuMemAddressFree(ptr, mem->size()));
  if (!status.ok()) {
    XLA_LOG_DEVICE(ERROR, device_ordinal())
        << "Failed to free virtual address " << mem->opaque()
        << " of size " << mem->size() << ": " << status;
  } else {
    XLA_VLOG_DEVICE(2, device_ordinal())
