...
class StreamExecutor {
 public:
  ...
  // DeviceAddressBase representing that reservation. In the case of failure,
  // nullptr is returned.
  virtual DeviceAddressBase ReserveAddress(uint64_t size,
                                           int64_t memory_space) = 0;
  DeviceAddressBase ReserveAddress(uint64_t size) {
    return ReserveAddress(size, /*memory_space=*/0);
  }

  // Frees the DeviceAddress previously reserved via this interface.
  // Deallocation of a nullptr-representative value is permitted.
  virtual void FreeAddress(DeviceAddressBase* mem) = 0;
  ...
  virtual absl::Status MapRawAddress(DeviceAddressBase* address, size_t offset,
                                     RawAddressHandle handle) = 0;

  // Unmaps raw physical memory from a virtual address.
  virtual absl::Status UnmapRawAddress(DeviceAddressBase* address) = 0;
